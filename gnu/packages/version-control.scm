  ...
  (package
    ...
    (properties
     '((check-in
       ...
       (revision . "0")))
    (version (fossil-version "0.19"
                             (assoc-ref properties 'revision)
                             (assoc-ref properties 'check-in)))
    (source
     (origin
       (method fossil-fetch)
       (uri (fossil-reference
             (uri "https://fnc.sh")
             (check-in (assoc-ref properties 'check-in))))
       (file-name (fossil-file-name name version))
       (sha256 (base32 "09m0bgzy0yjglsgrqq03sdq4p2rwi3y8m84979j066z1c5b444iz"))
       (modules '((guix build utils)))
       (snippet #~(begin
